...
#include <random>
#include <iostream>
// let try shared ptr, but I guess which will cause stack overflow
// 果然：0x000000013F3A4104 处有未经处理的异常(在 LinkedListRingDetector.exe 中): 0xC00000FD: Stack overflow (参数: 0x0000000000000001, 0x0000000000033F20)。
#include <memory>
using ull = unsigned long long;
struct HW_Node;
using spn = std::shared_ptr<HW_Node>;
struct HW_Node
{
...
bool isLoop(HW_LinkedListRing *linkedList)
{
	...
	static int maxNj = 0;
	// prepare for the while looping
	spn n1 = linkedList->GetHead(), n2 = linkedList->GetHead();
	while (n1 && n2)
	{
		n1 = n1->next;
		n2 = n2->GetNextNext();
